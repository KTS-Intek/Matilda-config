...
//-------------------------------------------------------------------------------------------------------------------------------------
ShowMessHelper4wdgt::ShowMessHelper4wdgt(QObject *parent) : QObject(parent)
{

}
//-------------------------------------------------------------------------------------------------------------------------------------
QString ShowMessHelper4wdgt::addWithFontColorRed(const QString m)
{
    return QString("<font color=red>%1</font>").arg(m);
}
//-------------------------------------------------------------------------------------------------------------------------------------
QString ShowMessHelper4wdgt::emailUtcOffset(const int offst)
{
    const QString h = QString::number(qAbs(offst)/3600).rightJustified(  2 , '0');
    const QString m = QString::number( ( qAbs(offst)%3600)/60).rightJustified(2, '0');
...
//-------------------------------------------------------------------------------------------------------------------------------------
QString ShowMessHelper4wdgt::addRez2endOfLine(const QString m, const bool &rezGood)
{
    return tr("%1. Operation: %2").arg(m).arg( rezGood ? tr("Done)") : addWithFontColorRed(tr("Failed(")) );
}
//-------------------------------------------------------------------------------------------------------------------------------------
QStringList ShowMessHelper4wdgt::recalcColumns(const QStringList &list, const QString spliter, const bool skipEmptyParts)
{
    QHash<int,int> hashColMaxLen;
    int maxCol = 0;
    const int iMax = list.size();
    for(int i = 0; i < iMax; i++){
        const QStringList l = list.at(i).split(spliter, skipEmptyParts ? QString::SkipEmptyParts : QString::KeepEmptyParts );
        int jMax = l.size();
        if(jMax > maxCol)
            maxCol = jMax;
...
//-------------------------------------------------------------------------------------------------------------------------------------
QString ShowMessHelper4wdgt::name4command(const quint16 &command)
{
    QString s;
    switch(command){
...
//-------------------------------------------------------------------------------------------------------------------------------------
QHash<QString, QString> ShowMessHelper4wdgt::columnKey2humanLang()
{
...
//    QStringList lHumanKeyMetric = tr("kW kW kvar kvar").split(" ");
    ...
    for(int i = 0, imax = lKey.size(); i < imax; i++)
        hKeyHuman.insert(lKey.at(i), tr("%1 [L]").arg(lHumanKey.at(i)));

    lKey = QString("F evnt_code comment date_time meter_sn meter_ni meter_model intrvl memo").split(" ");
    lHumanKey = tr("Frequency [Hz];Event code;Comment;Date and time;Serial Number;Meter Addres;Meter model;Interval;Memo").split(";");

    lKey.append(QString("crdnts"));
    lHumanKey.append(tr("Coordinate"));
    ...
    const QStringList listEnrg = QString("U I P Q cos_f").split(" ");
    const QStringList listPhs = QString("A B C").split(" ");

    const QStringList listEnrgHuman = tr("U;I;P;Q;Power factor").split(";");
    const QStringList listPhsHuman = tr("L1;L2;L3").split(";");
    const QStringList listMetricHuman = tr("V;A;kW;kvar;").split(";");
    ...
    lKey.append(QString("relay balance last_charge balance_charge balance_counter tarif_values params").split(" "));
    lHumanKey.append(tr("Relay;Balance;Last recharge;Recharge Balance;Recharge Counter;Values;Parameters").split(";"));

    lKey.append("deg");
    lHumanKey.append(tr("Temperature [%1]").arg(QString::fromUtf8("°C")));

    lKey.append(QString("stts msec").split(" "));
    lHumanKey.append(tr("Status;Time").split(";"));


    lKey.append(QString("meter_bat modem_bat meter_bat_v modem_bat_v profile_params").split(" "));
    lHumanKey.append(tr("Meter bat. time [min];Modem bat. time [min];Meter bat. [V]; Modem bat. [V];Water schedule").split(";"));

    for(int i = 0, iMax = qMax(lKey.size(), lHumanKey.size()); i < iMax; i++)
        hKeyHuman.insert(lKey.at(i), lHumanKey.at(i));

    return hKeyHuman;
}
//-------------------------------------------------------------------------------------------------------------------------------------
QHash<QString, QString> ShowMessHelper4wdgt::status2human()
{
    const QStringList lMachine = QString("ready;closing;closed;connecting").split(";");
    const QStringList lHuman = tr("Ready;Closing;Closed;Connecting").split(";");
...
//-------------------------------------------------------------------------------------------------------------------------------------
QHash<QString, QString> ShowMessHelper4wdgt::region2human()
{
    return QHash<QString, QString>();
}
//-------------------------------------------------------------------------------------------------------------------------------------
QHash<QString, QString> ShowMessHelper4wdgt::city2human()
{
    return QHash<QString, QString>();
}

//-------------------------------------------------------------------------------------------------------------------------------------
QHash<QString, QString> ShowMessHelper4wdgt::devCommand2human()
{
    return QHash<QString, QString>();
}
//-------------------------------------------------------------------------------------------------------------------------------------
QHash<QString, QString> ShowMessHelper4wdgt::manDevCommand2human()
{
    QStringList listDevCommand = QString("Custom date time, mask yyyy-MM-dd hh:mm|Depth interval,|"
                                         ...
                                         "Coma separeted energy keys. Default|"
                                         "Ignore existing poll data. Default: not ignored.").split("|");

    QStringList listDevCommandH = tr("Custom date and time, mask yyyy-MM-dd hh:mm|Depth interval,|"
                                     "space separeted NI list. Add model: <model>&<NI>. Add password: <NI>@<password>. Add model and password <model>&<NI>@<password>. If meter NI exists in device memory model and password can be omitted.|"
                                     "Max. tariff. By default|"
                                     "Coma separeted energy keys. Default|"
                                     "Ignore existing poll data. Default: not ignored.").split("|");
...
//-------------------------------------------------------------------------------------------------------------------------------------
QHash<QString, QString> ShowMessHelper4wdgt::devState2human()
{
    ...
    listDevCommand.append(QString("Suspended.|Resume.|Last code: |Wait for the signal ready. | [s]|"
                                  ...
                                  "Network reset").split("|"));
    listDevCommandH.append(tr("Suspended.|Resume.|Last code: |Waiting for the signal ready. | [s]|"
                              "Data from: |. model: |Date not valid. Current date: |Poll code |Checking the coordinator cofiguration.|"
                              "Can't enter command mode.|Check API+ mode.|Can't configure the modem.|Enabling API+ mode.|Mode |"
                              "Network reset").split("|"));
...
//-------------------------------------------------------------------------------------------------------------------------------------
QStringList ShowMessHelper4wdgt::devCode2txt(const int &devCode)
{
    QString s, ext;
    switch(devCode){
...
//-------------------------------------------------------------------------------------------------------------------------------------
QDateTime ShowMessHelper4wdgt::tableName2DateTime(const QString &tableName, const qint32 addDays, const bool useLocal)
{
...
//-------------------------------------------------------------------------------------------------------------------------------------
QDateTime ShowMessHelper4wdgt::tableName2DateTime(const QString &tableName, const QString &dateFormat, const QString &timeFormat, const qint32 addDays)
{
...
//-------------------------------------------------------------------------------------------------------------------------------------
QDateTime ShowMessHelper4wdgt::tableVal2DateTime(const QString &text, const QString &mask, const bool useLocal)
{
    //yyyy-MM-dd hh:mm:ss
...
//-------------------------------------------------------------------------------------------------------------------------------------
QString ShowMessHelper4wdgt::messFromCode(const int &messCode)
{
    QString mess;
    switch(messCode){
    ...
    case MESS_NO_ANSWER_FROM_DEVICE: mess = addWithFontColorRed(tr("No answer from the device(")); break;
...
//-------------------------------------------------------------------------------------------------------------------------------------
QString ShowMessHelper4wdgt::byte2humanRead(const qint64 &bytes)
{
    if(bytes > 10000000){
...
//-------------------------------------------------------------------------------------------------------------------------------------
QString ShowMessHelper4wdgt::addSpaceEvry(const QString &s, const int &evrySymb)
{
...
//-------------------------------------------------------------------------------------------------------------------------------------
QString ShowMessHelper4wdgt::str2hexStr(const QString &s)
{
    if(s.isEmpty())
        return "";
...
//-------------------------------------------------------------------------------------------------------------------------------------
QString ShowMessHelper4wdgt::prettyFileName4selFromDb(const QString &pName, const QDateTime &dtFrom, const QDateTime &dtTo, const bool &useLocalTime)
{
...
//-------------------------------------------------------------------------------------------------------------------------------------
QString ShowMessHelper4wdgt::prettyFileName4selFromDb(const QVariantHash &h, const bool &useLocalTime)
{
    return prettyFileName4selFromDb(h.value("pName").toString(), h.value("dtFrom").toDateTime(), h.value("dtTo").toDateTime(), useLocalTime);
}
//-------------------------------------------------------------------------------------------------------------------------------------
QStringList ShowMessHelper4wdgt::prettyTableShapka(const QVariantHash &h,  const QString &dateMask, const bool &useLocalTime)
{
...
//-------------------------------------------------------------------------------------------------------------------------------------
QString ShowMessHelper4wdgt::matildaFileName(const QString &fileName)
{
    QString m = tr("UC");
    if(fileName.left(m.length()) == m)
        ...
        m.append(" ");
    return QString("%1%2 %3").arg(m).arg(fileName).arg(QDateTime::currentDateTime().toString( "yyyy MM dd  hh mm" )).left(200)   ;
}
//-------------------------------------------------------------------------------------------------------------------------------------
QString ShowMessHelper4wdgt::prepaidFileName(const QString &fileName)
{
    QString m = tr("ND Prepaid");
    if(fileName.left(m.length()) == m)
        m.clear();
    else
        m.append(" ");
...
//-------------------------------------------------------------------------------------------------------------------------------------
QString ShowMessHelper4wdgt::timeZoneShortAbbreviation(const QDateTime &dt)
{    
    QString retVal = "";
...
//-------------------------------------------------------------------------------------------------------------------------------------
QString ShowMessHelper4wdgt::eventCode2human(const int &event)
{
    QStringList list;
    list.append(tr("All"));
...
//-------------------------------------------------------------------------------------------------------------------------------------
QHash<QString, QString> ShowMessHelper4wdgt::meterEv2human()
{
...
//-------------------------------------------------------------------------------------------------------------------------------------
QHash<QString, QString> ShowMessHelper4wdgt::matildaEv2human()
{
...
//-------------------------------------------------------------------------------------------------------------------------------------
QString ShowMessHelper4wdgt::translateString(QString inStr, const QHash<QString, QString> &hDev2human)
{
...
//-------------------------------------------------------------------------------------------------------------------------------------
QString ShowMessHelper4wdgt::meterEvCode2humanV2(const int &ucmEventCode)
{
    ...
    case UCM_EV_CASE_OPEN            : localEvntCode = tr("Meter cover: %1, %2").arg(tr("opened")).arg(tr("waiting for close"))                ; break;  //0x14 //Корпус лічильника відкритий
    ...
    case UCM_EV_VA_MAX_OVER          : localEvntCode = tr("Voltage L%1: %2, %3").arg("1").arg(tr("overvoltage")).arg(tr("waiting for normal")) ; break;  //0x41 //напруга Фази А вище порогу максимальної напруги
    ...
    case UCM_EV_VA_MIN_UNDER         : localEvntCode = tr("Voltage L%1: %2, %3").arg("1").arg(tr("undervoltage")).arg(tr("waiting for normal")); break;  //0x43 //Напруга Фази А нище порогу мінімальної напруги
    ...
    case UCM_EV_VB_MAX_OVER          : localEvntCode = tr("Voltage L%1: %2, %3").arg("2").arg(tr("overvoltage")).arg(tr("waiting for normal")) ; break;  //0x45 //напруга Фази B вище порогу максимальної напруги
    ...
    case UCM_EV_VB_MIN_UNDER         : localEvntCode = tr("Voltage L%1: %2, %3").arg("2").arg(tr("undervoltage")).arg(tr("waiting for normal")); break;  //0x47 //Напруга Фази B нище порогу мінімальної напруги
    ...
    case UCM_EV_VC_MAX_OVER          : localEvntCode = tr("Voltage L%1: %2, %3").arg("1").arg(tr("overvoltage")).arg(tr("waiting for normal")) ; break;  //0x49 //напруга Фази C вище порогу максимальної напруги
    ...
    case UCM_EV_VC_MIN_UNDER         : localEvntCode = tr("Voltage L%1: %2, %3").arg("3").arg(tr("undervoltage")).arg(tr("waiting for normal")); break;  //0x4B //Напруга Фази C нище порогу мінімальної напруги
    ...
    case UCM_EV_F_MAX_OVER           : localEvntCode = tr("Frequency: %1, %2").arg(tr("over")).arg(tr("waiting for normal"))                   ; break;  //0x4D //Частота вижче порогу нормальної
    ...
    case UCM_EV_F_MIN_UNDER          : localEvntCode = tr("Frequency: %1, %2").arg(tr("under")).arg(tr("waiting for normal"))                  ; break;  //0x4F //Частора нижче порогу нормальної
    ...
    case UCM_EV_T_MAX_OVER           : localEvntCode = tr("Temperature: %1, %2").arg(tr("high")).arg(tr("waiting for normal"))                 ; break;  //0x51 //Температура вище порогу максильної температури
    ...
    case UCM_EV_T_MIN_UNDER          : localEvntCode = tr("Temperature: %1, %2").arg(tr("low")).arg(tr("waiting for normal"))                  ; break;  //0x53 //Температура нижче порогу нормальної
    ...
    case UCM_EV_IA_MAX_OVER          : localEvntCode = tr("Current L%1: %2, %3").arg("1").arg(tr("high")).arg(tr("waiting for normal"))        ; break;  //0x55 //Струм Фази А вище порогу допустимого
    ...
    case UCM_EV_IB_MAX_OVER          : localEvntCode = tr("Current L%1: %2, %3").arg("2").arg(tr("high")).arg(tr("waiting for normal"))        ; break;  //0x57 //Струм Фази B вище порогу допустимого
    ...
    case UCM_EV_IC_MAX_OVER          : localEvntCode = tr("Current L%1: %2, %3").arg("3").arg(tr("high")).arg(tr("waiting for normal"))        ; break;  //0x59 //Струм Фази C вище порогу допустимого
    ...
    case UCM_EV_PA_MAX_OVER          : localEvntCode = tr("Power A+: %1, %2").arg(tr("high")).arg(tr("waiting for normal"))                    ; break;  //0x5B //Активна споживана потужність вище максимальної
    ...
    case UCM_EV_PV_MAX_OVER          : localEvntCode = tr("Power R+: %1, %2").arg(tr("high")).arg(tr("waiting for normal"))                    ; break;  //0x5D //Реактивна спожива потужність вище максимальної

    case UCM_EV_IDIFF_OK             : localEvntCode = tr("Idiff: %1").arg(tr("ok"))                                                        ; break;  //0x5E //Відновлення допустимого дифференціального струму
    case UCM_EV_IDIFF_OVER           : localEvntCode = tr("Idiff: %1, %2").arg(tr("over")).arg(tr("waiting for ok"))                           ; break;  //0x5F //Перевищення допустимого дифференціального струму
    case UCM_EV_CLOCK_OK             : localEvntCode = tr("Clock: %1").arg(tr("ok"))                                                        ; break;  //0x60 //Нормальний стан RTC відновлено
    case UCM_EV_CLOCK_FAULT          : localEvntCode = tr("Clock: %1, %2").arg(tr("error")).arg(tr("waiting for ok"))                          ; break;  //0x61 //RTC не встановлені
    ...
    case UCM_EV_CASE_KLEMA_OPEN      : localEvntCode = tr("Terminal box was %1, %2").arg(tr("opened")).arg(tr("waiting for close"))            ; break;   //0x96 //Клемна коробка лічильника відкрита
...
//    case UCM_EV_STANDARD_POWER_OUTAGES              : localEvntCode = UCM_EV_MSTATE_POWEROFF                                                   ; break; //           0x2003
    ...
    case UCM_EV_IN_MAX_OVER                         : localEvntCode = tr("Over current in neutral, %1").arg(tr("waiting for normal")); break;//  0x2917 //23 Over current in neutral
    ...
    case UCM_EV_PWR_QLT_VLTG_VSS_RCURR_ON           : localEvntCode = tr("Revers current detected at least in one of phases, %1").arg(tr("waiting for normal")); break;//           0x291E//30 Revers current detected at least in one of phases
...
//-------------------------------------------------------------------------------------------------------------------------------------
QStringList ShowMessHelper4wdgt::dataKeys2human(const QStringList &datalist)
{
    const QHash<QString,QString> hKeyHuman = ShowMessHelper4wdgt::columnKey2humanLang();
    const int iMax = datalist.size();
    QStringList l;
    for(int i = 0; i < iMax; i++)
        l.append(hKeyHuman.value(datalist.at(i), datalist.at(i)));
    return l;
}
//-------------------------------------------------------------------------------------------------------------------------------------
int ShowMessHelper4wdgt::calcPercent4thisInterval(const QVariantHash &hashMemo, const QVariantList &lastLine)
{
    return calcPercent4thisInterval(hashMemo, lastLine.first().toList().last().toDateTime().toUTC());
}
//-------------------------------------------------------------------------------------------------------------------------------------
int ShowMessHelper4wdgt::calcPercent4thisInterval(const QVariantHash &hashMemo, const QString &tableName)
{
    return calcPercent4thisInterval(hashMemo, ShowMessHelper4wdgt::tableName2DateTime(tableName));
}
//-------------------------------------------------------------------------------------------------------------------------------------
int ShowMessHelper4wdgt::calcPercent4thisInterval(const QVariantHash &hashMemo, QDateTime dateX)
{
    QDateTime fromDT = hashMemo.value("FromDT").toDateTime().toUTC();
    QDateTime toDT = hashMemo.value("ToDT").toDateTime().toUTC();

    if(!toDT.isValid())
        toDT = QDateTime::currentDateTimeUtc().toUTC();

    if(!fromDT.isValid())
        fromDT = toDT.addYears(-10);

    if(!dateX.isValid())
        dateX = fromDT;

    int dovzyna100p = qAbs(fromDT.daysTo(toDT));
    int dovzynaX = qAbs(fromDT.daysTo(dateX));

    if(dovzyna100p == 0)
        return 100;

    int rez = (dovzynaX * 100) / dovzyna100p;
